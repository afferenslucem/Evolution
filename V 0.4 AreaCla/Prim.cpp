...
void CreationC::_Step(UNI command)
    {
        ...
        case 0:                          //  1|2|3
            {   
                if(_IsClose(_x - 1, _y - 1) && _area[_y][_x]->GetType() == Wall) {
                    WallC* wall = (WallC*) _area[_y][_x];
                    _hp += wall->getHPC();; 
                    break;
                }//  -------
                if(_IsClose(_x - 1, _y - 1) && _area[_y][_x]->GetType() == Crt) {
                    CreationC* wall = (CreationC*) _area[_y][_x];
                    _hp += wall->getHPC();; 
                    break;
                }
                _x -= 1;           //  8|X|4
        ...
        case 1:
            {     
                if(_IsClose(_x, _y - 1) && _area[_y][_x]->GetType() == Wall) {
                    WallC* wall = (WallC*) _area[_y][_x];
                    _hp += wall->getHPC();; 
                    break;
                }//  -------
                if(_IsClose(_x, _y - 1) && _area[_y][_x]->GetType() == Crt) {
                    CreationC* wall = (CreationC*) _area[_y][_x];
                    _hp += wall->getHPC();; 
                    break;
                }           //  8|X|4
        ...
        case 3:
            {   
                if(_IsClose(_x + 1, _y) && _area[_y][_x]->GetType() == Wall) {
                    WallC* wall = (WallC*) _area[_y][_x];
                    _hp += wall->getHPC();; 
                    break;
                }//  -------
                if(_IsClose(_x + 1, _y) && _area[_y][_x]->GetType() == Crt) {
                    CreationC* wall = (CreationC*) _area[_y][_x];
                    _hp += wall->getHPC();; 
                    break;
                }
