...
//Creation
CreationC::CreationC(vector<UNI>& gens, Prim*** area, int x, int y):Prim(Crt)
{
    _itForCommand = 0;

    copy(gens.begin(), gens.end(), back_inserter(_commandList));
    _area = area;
    _y = y;
    _x = x;
    _head = up;
...
void CreationC::AddSlots(AreaCLA* obj)
{
    _eatingFood.connect(boost::bind(&AreaCLA::minFood, obj));
    _eatingPoison.connect(boost::bind(&AreaCLA::minPoison, obj));
    _creationDie.connect(boost::bind(&AreaCLA::delCrt, obj));
}   
